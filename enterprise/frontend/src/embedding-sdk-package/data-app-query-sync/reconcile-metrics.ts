...
function isMetricCard(card: { type: string }): card is DataAppMetricCard {
  return card.type === "metric";
}

function metricInput(metric: DataAppMetricCard, collectionId: number) {
  return {
    name: metric.name,
    collectionId,
...
export async function reconcileMetrics({
  ...
  for (const metric of metrics) {
    ...
    if (copied) {
      if (!isMetricCard(copied) || copied.collection_id !== collectionId) {
        throw new Error(
          `Metric copy Card ${copied.id} is no longer an owned metric in data app collection ${collectionId}.`,
        );
      }
      const changed =
        getPayloadFingerprint(metricInput(copied, collectionId)) !== hash;
      if (changed) {
        await client.updateMetric(copied.id, input);
      }
      copiedMetricIdBySourceMetricId[metric.id] = copied.id;
      entry.hash = hash;
      log(
        `${changed ? "updated" : "unchanged"} metric: card ${metric.id} -> card ${copied.id}`,
      );
    } else {
